...
router.get("/", function(req, res) {
    let sql = 'SELECT name,status FROM footer_config';
    // let params = [];
...
router.delete("/deletefooterconfiDataId/:footerconfiDataId?", (req, res) => {
  const accessconfiDataId = req.params.accessconfiDataId;
  var sql = 'DELETE FROM footer_config';
...
// Add purchase data
router.post("/insertfooterconfig", async (req, res) => {
  ...
  const sql = 'INSERT INTO footer_config (name , status) VALUES (?, ?)';
...
router.put("/updateFooterconfig", async (req, res) => {
  try {
    const { name } = req.body;
    ...
    const sql = "UPDATE footer_config SET status = CASE WHEN name IN (?) THEN 'active' ELSE 'inactive' END ";
    const values = [name];
    console.log(name)
