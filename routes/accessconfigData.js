...
router.get("/", function(req, res) {
    let sql = 'SELECT name,status FROM access_config  order by id';
    // let params = [];
...
router.delete("/deleteaccessconfiDataId/:accessconfiDataId?", (req, res) => {
  ...
  var sql = 'DELETE FROM access_config';
...
router.post("/insertaccessconfig", async (req, res) => {
  ...
  const sql = 'INSERT INTO access_config (name , status) VALUES (?, ?)';
...
router.put("/updateAccessconfig", async (req, res) => {
  try {
    ...
    const sql = "UPDATE access_config SET status = CASE WHEN name IN (?) THEN 'active' ELSE 'inactive' END ";
    const values = [name];
    console.log(name)
